...
import DecorativeRope from './DecorativeRope';
...
const BOX_COLORS = ['#800000', '#1B5E20', '#D4AF37', '#FFFFFF', '#4B0082', '#2F4F4F', '#DC143C'];
...
const SceneController: React.FC<{ 
    ...
    const rotationVelocity = useRef(0.0015);
    ...
    useEffect(() => {
        ...
        const onWheel = (e: WheelEvent) => {
            ...
            zoomTarget.current = THREE.MathUtils.clamp(zoomTarget.current, 15, 60);
        };
    ...
    useFrame((state, delta) => {
        ...
        const camX = currentInput.current.x * 3; 
        const camY = currentInput.current.y * 1.5; 
        const camZ = zoomTarget.current + Math.abs(currentInput.current.x) * 1.5; 
        camera.position.lerp(vec.set(camX, camY, camZ), 3.0 * safeDelta);
        camera.lookAt(0, 0, 0);
        ...
        if (groupRef.current) {
            if (isHandDetected) {
                ...
                if (wasDetected.current) {
                    if (Math.abs(rotationVelocity.current) < 0.0001) rotationVelocity.current = 0.0015; 
                    wasDetected.current = false;
                }
                if (!isDragging.current) {
                    groupRef.current.rotation.y += rotationVelocity.current;
                    rotationVelocity.current = THREE.MathUtils.lerp(rotationVelocity.current, 0.0015, safeDelta * 0.5);
                }
            }
        }
    });
...
const SceneContent: React.FC<ExperienceProps> = ({ mixFactor, colors, inputRef, userImages, allSignatures }) => {
  const groupRef = useRef<THREE.Group>(null);
  const photoCount = (userImages && userImages.length > 0) ? userImages.length : 12;
...
const Experience: React.FC<ExperienceProps> = (props) => {
  return (
    <Canvas
      dpr={[1, 1.5]} 
      camera={{ position: [0, 0, 32], fov: 45, near: 5, far: 100 }}
